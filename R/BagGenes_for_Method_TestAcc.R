#' @title BagGenes_for_Method_TestAcc
#'
...
BagGenes_for_Method_TestAcc<-function(fileInput=NULL,
                                      bagging.method=NULL,
                                      bagging.cutoff=0.75,
                                      save.folder=NULL,
                                      basic.fname=NULL,
  ...
  all.genes<-get(load())setdiff(colnames(DB.Train[[1]]$data.train),'ytrain')

  D.genes<-mclapply(DB.Train, function(l,all.genes){
    Mat<-sapply(l$genes, aux, all.genes=all.genes, simplify=TRUE, USE.NAMES=TRUE)
    Acc<-extractPerformance_bag(l)[,c(1,8)]
    pls.genes<-mapvalues(Mat[,'pls.genes'],from=1,to=Acc[which(Acc$method=='pls'),1])
    glm.genes<-mapvalues(Mat[,'glm.genes'],from=1,to=Acc[which(Acc$method=='glm'),1])
    pam.genes<-mapvalues(Mat[,'pam.genes'],from=1,to=Acc[which(Acc$method=='pam'),1])
    ensemble.genes<-mapvalues(Mat[,'ensemble.genes'],from=1,to=Acc[which(Acc$method=='ensemble'),1])
    ensemble.log.genes<-mapvalues(Mat[,'ensemble.genes'],from=1,to=Acc[which(Acc$method=='ensemble.log'),1])
    mat.acc<-cbind(pls.genes,glm.genes,pam.genes,ensemble.genes,ensemble.log.genes)
    },
  ...
  #   ggsave(GX,file= paste0(save.fname.fig,'_', m ,'_TestAcc_Selection_Freq.pdf'), width=6, height= (0.162*Num.Markers), limitsize = FALSE)
  # }

  write.csv(bagging.freq.genes, file=paste0(save.fname.table,'TestAcc_selection_frequencies.csv'),row.names = TRUE)
  save(bagging.genes=bagged.genes, file=paste0(save.fname.outputs,bagging.method,bagging.cutoff, '_TestAcc_BaggedGenes','.Rda'))
